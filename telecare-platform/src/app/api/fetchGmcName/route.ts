...
const extractSpecialties = (html: string): string[] => {
  const specialtyMatch = html.match(/<ul class="speciality-list">([\s\S]*?)<\/ul>/);
  if (!specialtyMatch) return [];
  
  const specialtyItems = specialtyMatch[1].match(/<span>(.*?)\s+from\s+/g) || [];
...
export async function GET(req: Request) {
  ...
  try {
    const response = await fetch(`https://www.gmc-uk.org/registrants/${gmcNumber}`);
    const html = await response.text();

    // Check for licence to practise
    if (!html.includes("Registered with a licence to practise")) {
      return NextResponse.json({ error: 'Invalid GMC - Not registered with a licence to practise' }, { status: 400 });
    }

    // Extract practitioner name
    const nameMatch = html.match(/<h1 class="c-rg-details__practitioner-name"[^>]*id="registrantNameId"[^>]*>(.*?)<\/h1>/);
    const fullName = nameMatch ? nameMatch[1].trim() : null;
    const nameData = fullName ? toCamelCase(fullName) : { firstName: "", lastName: "" };
    ...
    const fullRegPattern = html.match(/<div class="field-value">\s*Full registration date\s*<\/div>\s*<div class="field-value">\s*(\d{1,2}\s+(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s+\d{4})\s*<\/div>/s);
    if (fullRegPattern) {
    ...
    if (!registrationDate) {
      const altPattern = html.match(/Full registration date[^>]*>.*?(\d{1,2}\s+(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s+\d{4})/s);
      if (altPattern) {
    ...
    if (!registrationDate) {
      const datePattern = html.match(/(\d{1,2}\s+(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s+\d{4})/);
      if (datePattern) {
    ...
    const designatedBodyPattern = html.match(/<div class="field-label">\s*Designated body\s*<\/div>\s*<div class="field-value">\s*([^<]+?)\s*<\/div>/s);
    ...
    if (!institution || institution.length < 5) {
      const hospitalMatch1 = html.match(/<span[^>]*id="designated-body"[^>]*>.*?<\/span>\s*<div[^>]*class="c-rg-details__card-field"[^>]*>\s*([^<]+)/s);
      if (hospitalMatch1) {
    ...
    if (!institution || institution.length < 5) {
      const designatedBodyMatch = html.match(/designated\s+body[^>]*>.*?<div[^>]*>([^<]+)/is);
    ...
    return NextResponse.json({ error: 'Failed to fetch GMC data' }, { status: 500 });
  }
}