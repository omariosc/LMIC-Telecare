...
interface D1PreparedStatement {
  bind(..._params: unknown[]): D1PreparedStatement;
...
export class DatabaseHelper {
  ...
  async getUserById(id: string): Promise<User | null> {
    ...
    return {
      ...result,
      specialties: this.parseJsonField<string[]>(
        result.specialties as unknown as string
      ),
      verification_documents: this.parseJsonField<string[]>(
        result.verification_documents as unknown as string
      ),
      created_at: new Date(result.created_at),
      updated_at: new Date(result.updated_at),
      last_login_at: result.last_login_at
        ? new Date(result.last_login_at)
        : undefined,
      email_verified_at: result.email_verified_at
        ? new Date(result.email_verified_at)
        : undefined,
      verification_requested_at: result.verification_requested_at
        ? new Date(result.verification_requested_at)
        : undefined,
      verification_completed_at: result.verification_completed_at
        ? new Date(result.verification_completed_at)
        : undefined,
    };
  }
  ...
  async getCaseById(id: string): Promise<MedicalCase | null> {
    const stmt = this.db
      .prepare("SELECT * FROM medical_cases WHERE id = ?")
      .bind(id);
  ...
  async getCases(
    filters: CaseFilters = {}
  ): Promise<{ cases: CaseWithUser[]; total: number }> {
    let query = `
      SELECT mc.*, 
    ...
    if (filters.urgency && filters.urgency.length > 0) {
      conditions.push(
        `mc.urgency IN (${filters.urgency.map(() => "?").join(", ")})`
      );
      values.push(...filters.urgency);
    }
    if (filters.status && filters.status.length > 0) {
      conditions.push(
        `mc.status IN (${filters.status.map(() => "?").join(", ")})`
      );
      values.push(...filters.status);
    }
  ...
  async getResponseById(id: string): Promise<CaseResponse | null> {
    const stmt = this.db
      .prepare("SELECT * FROM case_responses WHERE id = ?")
      .bind(id);
  ...
  async getNotifications(
    ...
    const countStmt = this.db
      .prepare(`SELECT COUNT(*) as total FROM (${countQuery})`)
      .bind(...values);
    const countResult = await countStmt.first<{ total: number }>();

    return {
      notifications:
        result.results?.map(this.parseNotificationResult.bind(this)) || [],
      total: countResult?.total || 0,
    };
  }
  ...
  private parseCaseResult(result: any): MedicalCase {
    return {
      ...
      current_medications: this.parseJsonField<string[]>(
      ...
      translated_content: this.parseJsonField<Record<string, unknown>>(
