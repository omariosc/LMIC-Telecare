...
export default function RegistrationModal({
  ...
  const [gmcVerificationStatus, setGmcVerificationStatus] = useState<"pending" | "verified" | "failed">("pending");
  ...
  const [faceVerificationStatus, setFaceVerificationStatus] = useState<"pending" | "verified" | "failed">("pending");
  
  // NHS email verification states
  const [nhsEmail, setNhsEmail] = useState("");
  const [isVerifyingNHS, setIsVerifyingNHS] = useState(false);
  const [nhsVerificationStatus, setNhsVerificationStatus] = useState<"pending" | "code_sent" | "verified" | "failed">("pending");
  const [tfaCode, setTfaCode] = useState("");
  
  // Gaza clinician email verification states
  const [isVerifyingGazaEmail, setIsVerifyingGazaEmail] = useState(false);
  const [gazaEmailVerificationStatus, setGazaEmailVerificationStatus] = useState<"pending" | "code_sent" | "verified" | "failed">("pending");
  ...
  const verifyGMCNumber = async (gmcNumber: string) => {
    ...
    try {
      ...
      if (!response.ok || data.error) {
        setRegistrationError(language === "ar" ? "رقم GMC غير صحيح" : "Invalid GMC");
      ...
      setRegistrationError(language === "ar" ? "فشل في التحقق من رقم GMC" : "Failed to verify GMC number");
      setGmcVerificationStatus("failed");
    } finally {
  ...
  const verifyIDDocument = async (file: File) => {
    ...
    try {
      ...
      const gmcNames = [registrationData.firstName, registrationData.lastName].filter(name => name);
      const hasMatch = gmcNames.some(gmcName => 
        data.extractedNames?.some((extractedName: string) => 
      ...
      if (hasMatch) {
        ...
        setRegistrationError(language === "ar" ? "لم يتم العثور على تطابق في الأسماء مع وثيقة الهوية" : "No name match found in ID document");
      }
      
    } catch (error) {
      console.error('ID verification failed:', error);
      setIdVerificationStatus("failed");
      setRegistrationError(language === "ar" ? "فشل في التحقق من وثيقة الهوية" : "Failed to verify ID document");
    } finally {
  ...
  const captureFace = async () => {
    ...
    try {
      ...
      setRegistrationError(language === "ar" ? "فشل في التقاط الصورة" : "Failed to capture image");
      setIsCapturingFace(false);
    }
  };
  ...
  const verifyNHSEmail = async (email: string) => {
    ...
    if (!email.endsWith('@nhs.net')) {
      setRegistrationError(language === "ar" ? "يجب أن ينتهي البريد الإلكتروني بـ @nhs.net" : "Email must end with @nhs.net");
    ...
    try {
      ...
      setRegistrationError(language === "ar" ? "فشل في إرسال رمز التحقق" : "Failed to send verification code");
      setNhsVerificationStatus("failed");
    } finally {
      setIsVerifyingNHS(false);
    }
  ...
  const verify2FA = (code: string) => {
    if (code.length !== 6 || !/^\d{6}$/.test(code)) {
      setRegistrationError(language === "ar" ? "الرمز يجب أن يكون 6 أرقام" : "Code must be 6 digits");
      return;
    }
    ...
    if (storedCode === code && storedEmail === nhsEmail) {
      ...
      setRegistrationError(language === "ar" ? "رمز التحقق غير صحيح" : "Invalid verification code");
    }
  };
  ...
  const verifyGazaEmail = async (email: string) => {
    ...
    if (!email.trim()) {
      setRegistrationError(language === "ar" ? "البريد الإلكتروني مطلوب" : "Email is required");
    ...
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
      setRegistrationError(language === "ar" ? "البريد الإلكتروني غير صحيح" : "Invalid email address");
      setGazaEmailVerificationStatus("failed");
      setIsVerifyingGazaEmail(false);
      return;
    }
    ...
    if (registrationStep === 1) {
      if (registrationData.role === "uk_specialist") {
        ...
        if (!registrationData.gmcNumber.trim()) {
          setRegistrationError(language === "ar" ? "رقم GMC مطلوب" : "GMC number is required");
          return;
        }
        if (!/^\d{7}$/.test(registrationData.gmcNumber)) {
          setRegistrationError(language === "ar" ? "رقم GMC يجب أن يكون 7 أرقام فقط" : "GMC number must be exactly 7 digits");
          return;
        }
        if (gmcVerificationStatus !== "verified") {
          setRegistrationError(language === "ar" ? "يرجى انتظار التحقق من رقم GMC" : "Please wait for GMC verification to complete");
          return;
        }
        if (idVerificationStatus !== "verified") {
          setRegistrationError(language === "ar" ? "يجب التحقق من وثيقة الهوية أولاً" : "ID document verification required");
          return;
        }
        if (faceVerificationStatus !== "verified") {
          setRegistrationError(language === "ar" ? "يجب التحقق من صورة الوجه أولاً" : "Face verification required");
          return;
        }
        if (nhsVerificationStatus !== "verified") {
          setRegistrationError(language === "ar" ? "يجب التحقق من بريد NHS الإلكتروني أولاً" : "NHS email verification required");
          return;
        }
        // Validate password for UK specialists (shown after all verifications)
        if (!registrationData.password || !registrationData.confirmPassword) {
          setRegistrationError(language === "ar" ? "كلمة المرور مطلوبة" : "Password is required");
          return;
        }
        if (registrationData.password !== registrationData.confirmPassword) {
        ...
        if (!registrationData.referralCode.trim()) {
          setRegistrationError(language === "ar" ? "رمز الإحالة مطلوب" : "Referral code is required");
          return;
        }
        if (registrationData.referralCode !== "DeenDevelopers") {
          setRegistrationError(language === "ar" ? "رمز الإحالة غير صحيح" : "Invalid referral code");
          return;
        }
        // Validate email verification for Gaza clinicians
        if (gazaEmailVerificationStatus !== "verified") {
          setRegistrationError(language === "ar" ? "يجب التحقق من البريد الإلكتروني أولاً" : "Email verification required");
          return;
        }
  ...
  const handleRegistrationSubmit = () => {
    ...
    const isGazaWithValidCode = registrationData.role === "gaza_clinician" && registrationData.referralCode === "DeenDevelopers";
    
    const newRequest = {
      id: `pending_${Date.now()}`,
      ...registrationData,
      // For UK specialists, use NHS email as primary email
      email: registrationData.role === "uk_specialist" ? nhsEmail : registrationData.email,
        ...
        {registrationStep === 1 && (
          <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 gap-3">
                <label className={`flex items-center ${
                  ...
                  <input
                    ...
                    onChange={(e) => setRegistrationData({...registrationData, role: e.target.value as "uk_specialist" | "gaza_clinician"})}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setRegistrationData({...registrationData, gmcNumber: value});
                    ...
                    placeholder={language === "ar" ? "أدخل رقم GMC (7 أرقام)" : "Enter GMC number (7 digits)"}
                    maxLength={7}
                  />
                </div>
                  ...
                  <>
                    ...
                    {gmcVerificationStatus === "verified" && (
                      <div className="bg-green-50 dark:bg-green-950 p-3 rounded-lg">
                        ...
                        <div className="text-xs text-green-700 dark:text-green-300 space-y-1">
                          <p><strong>{language === "ar" ? "الاسم:" : "Name:"}</strong> {registrationData.firstName} {registrationData.lastName}</p>
                          <p><strong>{language === "ar" ? "المؤسسة:" : "Institution:"}</strong> {registrationData.institution}</p>
                          <p><strong>{language === "ar" ? "سنوات الخبرة:" : "Years of Experience:"}</strong> {registrationData.yearsOfExperience}</p>
                          <p><strong>{language === "ar" ? "التخصصات:" : "Specialties:"}</strong> {registrationData.specialties?.join(", ")}</p>
                        </div>
                      </div>
                    )}
                    ...
                    {gmcVerificationStatus === "verified" && (
                      <>
                        <div>
                          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                            {language === "ar" ? "وثيقة الهوية (جواز السفر أو رخصة القيادة)" : "ID Document (Passport or Driving License)"}
                          </label>
                          <input
                            type="file"
                            accept="image/*"
                            onChange={(e) => {
                              ...
                              if (file) {
                                ...
                                reader.onload = (e) => setIdImagePreview(e.target?.result as string);
                          ...
                          {idImagePreview && (
                            <div className="mt-2">
                              <img src={idImagePreview} alt="ID Preview" className="w-32 h-20 object-cover rounded" />
                            </div>
                          )}
                        </div>
                        ...
                        {isVerifyingID && (
                          <div className="bg-yellow-50 dark:bg-yellow-950 p-3 rounded-lg">
                            <p className="text-sm text-yellow-800 dark:text-yellow-200">
                              {language === "ar" ? "جارٍ التحقق من وثيقة الهوية..." : "Verifying ID document..."}
                            </p>
                          </div>
                        )}
                        ...
                        {idVerificationStatus === "verified" && (
                          <div className="bg-green-50 dark:bg-green-950 p-3 rounded-lg">
                            <p className="text-sm text-green-800 dark:text-green-200 mb-2">
                              {language === "ar" ? "تم التحقق من وثيقة الهوية بنجاح!" : "ID document verified successfully!"}
                            </p>
                            {extractedNames.length > 0 && (
                              <p className="text-xs text-green-700 dark:text-green-300">
                                {language === "ar" ? "الأسماء المستخرجة:" : "Extracted names:"} {extractedNames.join(", ")}
                              </p>
                            )}
                          </div>
                        )}
                        ...
                        {idVerificationStatus === "failed" && (
                          <div className="bg-red-50 dark:bg-red-950 p-3 rounded-lg">
                            <p className="text-sm text-red-800 dark:text-red-200">
                              {language === "ar" ? "فشل في التحقق من وثيقة الهوية" : "Failed to verify ID document"}
                            </p>
                          </div>
                        )}
                      </>
                    ...
                    {idVerificationStatus === "verified" && (
                      <>
                        <div>
                          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                            {language === "ar" ? "التحقق من الوجه" : "Face Verification"}
                          </label>
                          <button
                            type="button"
                            onClick={captureFace}
                            disabled={isCapturingFace || faceVerificationStatus === "verified"}
                            className="w-full bg-blue-600 text-white py-2 px-4 rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                          >
                            {isCapturingFace 
                              ? (language === "ar" ? "جارٍ التقاط الصورة..." : "Capturing...")
                              : faceVerificationStatus === "verified"
                                ? (language === "ar" ? "تم التحقق من الوجه" : "Face Verified")
                                : (language === "ar" ? "التقاط صورة الوجه" : "Capture Face Photo")
                            }
                          </button>
                          {faceImage && (
                            <div className="mt-2">
                              <img src={faceImage} alt="Face capture" className="w-32 h-32 object-cover rounded" />
                            </div>
                          )}
                        </div>
                        ...
                        {faceVerificationStatus === "verified" && (
                          <div className="bg-green-50 dark:bg-green-950 p-3 rounded-lg">
                            <p className="text-sm text-green-800 dark:text-green-200">
                              {language === "ar" ? "تم التحقق من الوجه بنجاح!" : "Face verification successful!"}
                            </p>
                          </div>
                        )}
                      </>
                    ...
                    {faceVerificationStatus === "verified" && (
                      <>
                        <div>
                          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                            {language === "ar" ? "بريد NHS الإلكتروني" : "NHS Email Address"}
                          </label>
                          <div className={`flex gap-2 ${
                            ...
                            <input
                              ...
                              placeholder={language === "ar" ? "your.name@nhs.net" : "your.name@nhs.net"}
                              disabled={nhsVerificationStatus === "verified"}
                            />
                            <button
                              type="button"
                              onClick={() => verifyNHSEmail(nhsEmail)}
                              disabled={isVerifyingNHS || nhsVerificationStatus === "verified" || nhsVerificationStatus === "code_sent"}
                        ...
                        {nhsVerificationStatus === "verified" && (
                          <div className="bg-green-50 dark:bg-green-950 p-3 rounded-lg">
                            <p className="text-sm text-green-800 dark:text-green-200">
                              {language === "ar" ? "تم التحقق من البريد الإلكتروني بنجاح!" : "Email verification successful!"}
                            </p>
                          </div>
                        )}
                      </>
                      ...
                      <>
                        <div className="bg-green-50 dark:bg-green-950 p-3 rounded-lg">
                          <p className="text-sm text-green-800 dark:text-green-200 font-medium">
                            {language === "ar" ? "تم التحقق من جميع البيانات! يمكنك الآن إنشاء كلمة المرور" : "All verifications complete! You can now create your password"}
                          </p>
                        </div>
              ...
              <>
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 border border-gray-300 dark:border-zinc-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 dark:bg-zinc-800 dark:text-white"
                    placeholder={language === "ar" ? "أدخل رمز الإحالة" : "Enter referral code"}
                  />
                </div>
                  ...
                  <>
                    ...
                    <div>
                      ...
                      <div className={`flex gap-2 ${
                        ...
                        <button
                          type="button"
                          onClick={() => verifyGazaEmail(registrationData.email)}
                          disabled={isVerifyingGazaEmail || !registrationData.email.trim() || gazaEmailVerificationStatus === "verified"}
                        ...
                        >
                          {isVerifyingGazaEmail
                            ...
                            : gazaEmailVerificationStatus === "verified"
                              ...
                              : (language === "ar" ? "إرسال رمز" : "Send Code")
                          }
                        </button>
                      </div>
                    </div>
                    ...
                    {gazaEmailVerificationStatus === "verified" && (
                      <>
                        <div className="bg-green-50 dark:bg-green-950 p-3 rounded-lg">
                          <p className="text-sm text-green-800 dark:text-green-200 font-medium">
                            {language === "ar" ? "تم التحقق من البريد الإلكتروني! يمكنك الآن إنشاء كلمة المرور" : "Email verified! You can now create your password"}
                          </p>
                        </div>
            ...
            <div className={`flex gap-3 ${
              ...
              <button
                onClick={
                  (registrationData.role === "gaza_clinician" && registrationData.referralCode === "DeenDevelopers" && gazaEmailVerificationStatus === "verified") ||
                  (registrationData.role === "uk_specialist" && 
                   gmcVerificationStatus === "verified" && 
                   idVerificationStatus === "verified" && 
                   faceVerificationStatus === "verified" && 
                ...
                disabled={isVerifyingGMC || isVerifyingID || isCapturingFace || isVerifyingNHS || isVerifyingGazaEmail}
              ...
              >
                {isVerifyingGMC || isVerifyingID || isCapturingFace || isVerifyingNHS || isVerifyingGazaEmail
