...
type RegistrationStep = "gmc" | "passport" | "camera" | "twofa" | "account" | "processing" | "result";
...
const STEPS = [
  { id: "gmc", title: "GMC Verification", description: "Verify your GMC registration" },
  { id: "passport", title: "Passport Upload", description: "Upload your passport photo page" },
  { id: "camera", title: "Face Verification", description: "Take a live photo for verification" },
  { id: "twofa", title: "Email Verification", description: "Verify your email address" },
  { id: "account", title: "Account Setup", description: "Create your account credentials" },
] as const;
...
const RegistrationPage = () => {
  ...
  // Passport Step
  const [passportImage, setPassportImage] = useState<{ file: File; dataUrl: string } | null>(null);
  
  // Camera Step
  const [livePhoto, setLivePhoto] = useState<{ blob: Blob; dataUrl: string } | null>(null);
  ...
  useEffect(() => {
    const loadModels = async () => {
      try {
        ...
        console.warn("Face detection models not available. Face verification will be simulated.");
  ...
  const fetchGMCName = async (gmcNumber: string) => {
    ...
    try {
      ...
      alert("Failed to fetch GMC name. Please check your GMC number and try again.");
      console.error(err);
    } finally {
      setGmcLoading(false);
    }
  ...
  const extractPassportData = async (imageFile: File): Promise<string> => {
    try {
      ...
      if (mrzMatch) {
        ...
        const findAfterKeyword = (keywords: string[], pattern = /([A-Z\s]+)/i) => {
  ...
  const processVerification = async () => {
    ...
    try {
      ...
      const verified = gmcName && passportName !== "Unknown" && faceComparison.match;
    ...
    <div className="mb-8">
      <div className="flex items-center justify-between max-w-4xl mx-auto">
        {STEPS.map((stepInfo, index) => {
          ...
          const isCompleted = index < stepIndex || (step === "result" && verificationResult?.verified);
          ...
          return (
            <div key={stepInfo.id} className="flex-1 flex items-center">
              <div className="flex flex-col items-center">
                <button
                  onClick={() => isAccessible ? moveToStep(stepInfo.id as RegistrationStep) : undefined}
  ...
  const renderGMCStep = () => (
    <div className="max-w-md mx-auto">
      <div className="bg-white rounded-lg shadow-lg p-6" style={{ borderColor: "var(--primary-blue)" }}>
        <div className="text-center mb-6">
          <div className="w-12 h-12 bg-blue-100 rounded-full flex items-center justify-center mx-auto mb-4">
            <User className="w-6 h-6 text-blue-600" />
          </div>
          <h2 className="text-2xl font-bold mb-2" style={{ color: "var(--primary-blue)" }}>
            GMC Verification
          </h2>
    ...
    <div className="max-w-md mx-auto">
      <div className="bg-white rounded-lg shadow-lg p-6">
        <div className="text-center mb-6">
          ...
          <p className="text-gray-600">
            Please upload a clear picture of your passport photo page for identity verification
          </p>
        </div>
        ...
        <div className="space-y-4">
          ...
          >
            ...
            <span className="text-gray-600">Click to upload passport photo page</span>
          </button>
    ...
    <div className="max-w-lg mx-auto">
      <div className="bg-white rounded-lg shadow-lg p-6">
        <div className="text-center mb-6">
          ...
          <p className="text-gray-600">
            Please look directly at the camera and capture a clear photo of your face
          </p>
        </div>
          ...
          {verificationResult?.verified ? (
            <>
              ...
              <p className="text-gray-600 mb-6">
                Welcome to Jusur, Dr. {gmcName}. Your account has been created successfully.
              </p>
              
              <div className="bg-green-50 rounded-lg p-4 mb-6">
                <div className="text-left text-sm space-y-2">
                  <div><span className="font-semibold">Email:</span> {email}</div>
                  <div><span className="font-semibold">GMC Name:</span> {gmcName}</div>
                  <div><span className="font-semibold">Verification Status:</span> ✅ Verified</div>
                </div>
              </div>
            ...
            <>
              ...
              <p className="text-gray-600 mb-6">
                {verificationResult?.error || "Could not verify your identity. Please try again."}
              </p>
                ...
                <div className="bg-red-50 rounded-lg p-4 mb-6 text-left text-sm">
                  ...
                  <div className="space-y-1">
                    ...
                    {verificationResult.passportData && (
                      <div><span className="font-semibold">Passport Name:</span> {verificationResult.passportData.name}</div>
                    )}
                    {verificationResult.faceMatch && (
                      <div><span className="font-semibold">Face Match:</span> {Math.round(verificationResult.faceMatch.confidence * 100)}%</div>
                    )}
                  </div>
                </div>
              )}
  ...
  return (
    <div className="min-h-screen py-8" style={{ backgroundColor: "var(--light-gray)" }}>
