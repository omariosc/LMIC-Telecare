...
export default function EnhancedRegistrationModal({
  ...
  const verifyGMC = async () => {
    ...
    if (registrationData.gmcNumber.length !== 7 || !/^\d{7}$/.test(registrationData.gmcNumber)) {
      setError(
        language === "ar" ? "رقم GMC يجب أن يكون 7 أرقام بالضبط" : "GMC number must be exactly 7 digits"
      );
      return;
    }
  ...
  const handleIdUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (e) => {
      ...
      try {
        ...
        // Extract name from the text and validate against GMC data
        const extractedText = text.toLowerCase();
        const nameWords = [
          registrationData.firstName.toLowerCase(),
          registrationData.lastName.toLowerCase(),
        ].filter((word) => word.length > 2); // Only check words longer than 2 characters

        // Check if ANY word from the name appears in the extracted text
        ...
        if (foundWords.length > 0) {
          ...
          setError(
            language === "ar"
              ? "لا يمكن التحقق من الهوية. تأكد من أن الاسم يطابق سجل GMC"
              : "Cannot verify ID. Ensure name matches GMC record"
          );
        }
      } catch (error) {
        setIdVerificationStatus("failed");
        setError(
          language === "ar" ? "فشل في قراءة الهوية" : "Failed to read ID"
        );
      }
    };
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-lg max-w-md w-full max-h-[90vh] overflow-y-auto flex flex-col">
        ...
        <div className="p-6 flex-1 overflow-y-auto">
                ...
                <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-4">
                  ...
                  <ol className="text-sm text-green-700 dark:text-green-300 space-y-2">
                    <li className="flex items-start gap-3">
                      <span className="text-xs bg-green-200 dark:bg-green-800 text-green-800 dark:text-green-200 rounded-full w-6 h-6 flex items-center justify-center font-bold flex-shrink-0 mt-0.5">1</span>
                      <span>
                        {language === "ar"
                          ? "التحقق من رقم GMC"
                    ...
                    <li className="flex items-start gap-3">
                      <span className="text-xs bg-green-200 dark:bg-green-800 text-green-800 dark:text-green-200 rounded-full w-6 h-6 flex items-center justify-center font-bold flex-shrink-0 mt-0.5">2</span>
                      <span>
                        {language === "ar"
                          ? "رفع صورة جواز السفر"
                    ...
                    <li className="flex items-start gap-3">
                      <span className="text-xs bg-green-200 dark:bg-green-800 text-green-800 dark:text-green-200 rounded-full w-6 h-6 flex items-center justify-center font-bold flex-shrink-0 mt-0.5">3</span>
                      <span>
                        {language === "ar"
                    ...
                    <li className="flex items-start gap-3">
                      <span className="text-xs bg-green-200 dark:bg-green-800 text-green-800 dark:text-green-200 rounded-full w-6 h-6 flex items-center justify-center font-bold flex-shrink-0 mt-0.5">4</span>
                      <span>
                        {language === "ar"
                    ...
                    <li className="flex items-start gap-3">
                      <span className="text-xs bg-green-200 dark:bg-green-800 text-green-800 dark:text-green-200 rounded-full w-6 h-6 flex items-center justify-center font-bold flex-shrink-0 mt-0.5">5</span>
                      <span>
                        {language === "ar"
                          ? "إنشاء كلمة المرور"
                          : "Account Setup"}
          ...
          {currentPage === "gmc" && (
            <div className="space-y-4">
              ...
              <button
                ...
                disabled={gmcVerificationStatus === "verifying" || (gmcVerificationStatus !== "verified" && registrationData.gmcNumber.length !== 7)}
          ...
          {currentPage === "id" && (
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                ...
                {(idVerificationStatus === "failed" || uploadedIdImage) && idVerificationStatus !== "verified" && (
                  <button
                    onClick={() => {
                      setError("");
                      setIdVerificationStatus("verified");
                  ...
                  >
                    {language === "ar" ? "تخطي التحقق من الهوية" : "Skip ID Verification"}
                  </button>
                )}
              </div>
            </div>
          ...
          {currentPage === "camera" && (
            <div className="space-y-4">
              <h3 className="text-lg font-medium text-gray-900 dark:text-white">
                {language === "ar" ? "التقاط صورة شخصية" : "Take Profile Photo"}
              </h3>
              ...
              <div className="flex justify-center space-x-2">
                {tfaCode.map((digit, index) => (
                  <input
                    ...
                    type="text"
                    value={digit}
                    onChange={(e) => handleTfaCodeChange(index, e.target.value)}
                    className="w-12 h-12 text-center text-lg font-medium border border-gray-300 dark:border-zinc-600 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent dark:bg-zinc-800 dark:text-white"
                    maxLength={1}
                  />
                ))}
              </div>
